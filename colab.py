...
if r.ping():
    cprint('Connected to Redis', 'green', 'on_light_cyan')
else:
    cprint('Redis Connection Failed', 'red', 'on_light_cyan')
...
LP_LOCKED_PCT_THRESHOLD = 57 #Only buys tokens that have a locked liquidity of less than 10%
...
WSS = "ws://api.mainnet-beta.solana.com"  # "wss://api.devnet.solana.com" | "wss://api.mainnet-beta.solana.com"
...
async def main():
	...
	cprint("Starting...", 'red', 'on_white')
	async for websocket in connect(WSS):
		try:
			cprint("Connected to Solana", 'green', 'on_white')
			...
			cprint("Subscribed to RaydiumLPV4", 'green', 'on_white')
			#call calc_lpLockedPct
			...
			async for i, signature in enumerate(process_messages(websocket, log_instruction)):  # type: ignore
				...
				try:
					...
					cprint(f"Processing message {i}", 'yellow', 'on_green')

					await get_tokens(signature, RaydiumLPV4)  # AttributeError: 'NoneType' object has no attribute 'transaction'

				except (AttributeError, SolanaRpcException, ValueError) as err:
			...
			if websocket:
				cprint("Unsubscribing...", 'yellow', 'on_white')
...
async def process_messages(websocket: SolanaWsClientProtocol,
	...
	async for idx, msg in enumerate(websocket):
		...
		if not idx % 100:
			print(colored(f"# of indexes: {idx}", 'yellow', 'on_red'))
...
#calculate locked liquidity pct
...
async def parsePoolInfoFromLpTransaction(
	...
	try:	
		...
		print(f"Error in parsePoolInfoFromLpTransaction: {e}")	


	withdrawQueue = str("11111111111111111111111111111111")
	r.set('withdrawQueue', withdrawQueue)
	...
	print("STARTING TO PARSE BASE AND QUOTE DECIMALS")
	try:
	...
	try:
		...
		print(f"Error in openTime section: {e}")
	...
	for x in innerInstructions_0:
		for y in x.instructions:
			try:
				if str(y.parsed['info']['mint']) == accounts[7] and y.parsed['type'] == 'initializeMint':
					lpDecimals = y.parsed['info']['decimals']
				elif str(y.parsed['info']['mint']) == accounts[7] and y.parsed['type'] == 'mintTo':
					lpReserve = y.parsed['info']['amount']
					lpReserve_ = y.parsed['info']['amount']
				else:
					pass
	...
	# if liquidity_locked_pct is None:
	# 	liquidity_locked_pct = 0
	# else:
	if liquidity_locked_pct is not None:
		liquidity_locked_pct = int(float(liquidity_locked_pct))
	else:
		# Handle the case when liquidity_locked_pct is None
...
# Start logging to transactions.json
	...
	# End logging
	instructions = await get_instructions(transaction)
		...
		if open_time > current_ts:
			...
			try:
				if t_diff < WHEN_TO_BUY:
					...
					print(f"Time difference is greater than {WHEN_TO_BUY} seconds. Not buying.")
			except Exception as e:
